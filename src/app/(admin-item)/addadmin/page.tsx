...
const Page = () => {
    ...
    const handleUserAdd = async (e: FormEvent<HTMLFormElement>) => {
        ...
        setPending(true);
        try {
            const response = await fetch(`${apiBaseUrl}/auth/addAdminUser`, {
                method: 'POST',
                headers: {
            ...
            <div className="flex w-full items-center justify-center pt-10">
                <div className="card shadow shadow-slate-700 w-full max-w-sm p-5">
                    <form onSubmit={handleUserAdd}>
                        <h1 className='font-bold tracking-widest p-2'>ADD ADMIN USER</h1>
                        <div className="flex flex-col">
                            ...
                            <div className="flex p-2">
                                <button type='submit' className='btn btn-success w-full max-w-xs'>{pending ? "Submitting..." : "ADD ADMIN"}</button>
                            </div>
                        </div>
                    </form>
                </div>
